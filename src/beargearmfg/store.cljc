(ns beargearmfg.store
  "SSoT for the bearings/gears/gearing/driving-elements plant-operations
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2710`'s own
  `elecequipmfg.store`, `cloud-itonami-isic-2818`'s own
  `powertoolmfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external bearing/gear-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/tolerance-test-um/quantity/
    ...
    - `equipment`         -- a machining-line/grinding-line unit's own
    ...
    - `maintenance`       -- a scheduled machining/grinding-line
                             maintenance-window DRAFT against a piece
                             of equipment (`beargearmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound bearing/gear shipment
                             DRAFT (`beargearmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (beargearmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (beargearmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :ball-bearing
                :model "BB-6205" :tolerance-test-um 8.0
                :quantity-units 5000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :spur-gear
                :model "SG-M2-24T" :tolerance-test-um 15.0
                :quantity-units 1200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 1150.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :roller-bearing
                :model "RB-32210" :tolerance-test-um 12.0
...
(defn- sample-equipment []
  {"mach-001" {:id "mach-001" :kind :precision-machining-line
               :verified? true :registered? true
               :last-maintenance-date "2026-05-01"}
   "grind-002" {:id "grind-002" :kind :grinding-line
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `beargearmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `beargearmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  precision-machining-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered grinding-line unit (blocks any maintenance

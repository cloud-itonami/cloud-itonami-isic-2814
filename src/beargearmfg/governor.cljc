(ns beargearmfg.governor
  "Bearings, Gears and Driving Elements Plant Operations Governor -- the
  independent compliance layer that earns the BearGearAdvisor the
  ...
  draft-schedule) machining/grinding-line equipment, whether a
  proposal secretly tries to self-issue a bearing/gear tolerance-class
  ...
  coordination proposal and becomes direct machining/grinding-line-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:bearings-gears-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:machining-line/
                                       actuate` or `:grinding-line/
                                       run`) is the 'direct machining/
                                       grinding-line-equipment control'
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       bearing/gear tolerance-class
                                       certification mark (e.g. ISO 492
                                       bearing tolerance class or
                                       AGMA/ISO 1328 gear quality) --
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [beargearmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  grinding-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct machining/grinding-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は精密機械加工・研削設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate machining/grinding-line equipment -- this actor
  may only ever propose/schedule a DRAFT maintenance window, never
  actuate the equipment directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-equipment? (:value proposal))))
    [{:rule :equipment-actuate-blocked
      :detail "精密機械加工・研削設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a bearing/gear tolerance-class certification mark
  (e.g. ISO 492 bearing tolerance class or AGMA/ISO 1328 gear quality)
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "軸受・歯車公差等級認証(ISO 492/AGMA/ISO 1328等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-tolerance-test-um-violations
  "For `:log-production-batch`, if the patch declares a
  `:tolerance-test-um` that is not a physically plausible dimensional-
  tolerance test reading, reject rather than let fabricated/sensor-
  error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [um (:tolerance-test-um (:value proposal))]
      (when (and (some? um) (not (registry/tolerance-test-um-valid? um)))
        [{:rule :invalid-tolerance-test-um
          :detail (str um "um は物理的に妥当な寸法公差試験値の範囲外")}]))))
...
(defn check
  "Censors a BearGearAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-tolerance-test-um-violations request proposal)

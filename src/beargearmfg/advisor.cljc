(ns beargearmfg.advisor
  "BearGearAdvisor -- the *contained intelligence node* for the
  bearings/gears/gearing/driving-elements plant-operations coordination
  actor.

  It normalizes production-batch patches (product-type/tolerance-test-
  um/quantity/defect-rate), drafts a machining/grinding-line-equipment
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound bearing/gear shipment
  ...
  a real machining/grinding-line-equipment actuation, freight dispatch,
  or tolerance-class certification -- see README `What this actor does
  NOT do`. Every output is censored downstream by `beargearmfg.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `beargearmfg.governor`)
  ...
  `beargearmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  tolerance-test-um, quantity, defect-rate or verification status.
...
(defn- schedule-maintenance
  "Draft a machining/grinding-line-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `beargearmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a materials-safety/equipment-safety concern. ALWAYS `:stake
  ...
  `beargearmfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `beargearmfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound bearing/gear shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit quantity -- it does NOT invent one, and
  `beargearmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは軸受・歯車(ボールベアリング・ローラーベアリング・平歯車・"
       "はすば歯車・ウォームギア等の伝動要素)製造工場プラント運用コーディ"
       "ネーターの助言者です。"
       ...
       "精密機械加工・熱処理・研削等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "軸受・歯車の公差等級認証(ISO 492/AGMA/ISO 1328等)を自己発行する提案を"
       "してはいけません。出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `beargearmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :beargearmfg-advisor-proposal
   :op         (:op request)

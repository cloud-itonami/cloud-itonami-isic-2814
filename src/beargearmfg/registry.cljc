(ns beargearmfg.registry
  "Pure-function domain logic for the bearings/gears/gearing/driving-
  elements plant-operations coordination actor -- equipment/batch
  verification, shipment-quantity recompute, product-type validation,
  dimensional-tolerance plausibility validation, defect-rate
  ...
  pre-existing `kotoba-lang/beargearmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `beargearmfg.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `elecequipmfg.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2710`, and `powertoolmfg.registry` from
  `cloud-itonami-isic-2818`): never trust a proposal's own
  ...
  coordinated shipment), not the act of actuating machining/grinding-
  line equipment or dispatching a real freight carrier, and never the
  act of issuing a bearing/gear tolerance-class certification mark
  (this actor NEVER does any of those -- see README `What this actor
  does NOT do`).

  SCOPE: ISIC 2814 covers manufacture of bearings, gears, gearing and
  driving elements -- precision-machining, heat-treatment, and
  grinding lines producing finished ball bearings, roller bearings,
  spur gears, helical gears and worm gears (transmission/driving-
  element components). This actor coordinates the back-office
  record-keeping around that plant (production-batch logging,
  maintenance scheduling, safety-concern flagging, shipment
  coordination) -- it never touches the machining/grinding-line
  equipment directly, and it never stands in for the certification
  body that issues bearing/gear tolerance-class conformance marks
  (e.g. ISO 492 / AGMA / ISO 1328).")
...
(def valid-product-types
  ...
  through."
  #{:ball-bearing :roller-bearing :spur-gear :helical-gear :worm-gear})

(def tolerance-test-um-min
  "Physical floor for a batch's own dimensional-tolerance test reading,
  in micrometers (um). A measured deviation is never negative."
  0.0)

(def tolerance-test-um-max
  "Physical ceiling for a batch's own dimensional-tolerance test
  reading, in micrometers (um). Informed by ISO 492 bearing tolerance
  classes (P0 through P2, single-digit-to-tens-of-micrometer runout)
  and AGMA/ISO 1328 gear quality grades (base-pitch/profile error that
  can run into the low hundreds of micrometers for coarser grades) --
  this ceiling generously bounds well above any real routine
  dimensional-tolerance measurement on any standardized class of
  bearing or gear; a reading above it is implausible sensor/QC data,
  not a real measurement."
  300.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own machining/heat-treatment/grinding
  defect-rate reading (zero defects is the best possible outcome,
  never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own machining/heat-treatment/
  grinding defect-rate reading -- a batch cannot reject more than 100%
...
(defn batch-verified?
  ...
  (i.e. its product-type/tolerance-test-um/quantity/defect-rate
...
(defn tolerance-test-um-valid?
  "Is `um` a physically plausible dimensional-tolerance test reading,
  in micrometers? Rejects nil, non-numbers, negative values, and
  values beyond `tolerance-test-um-max` -- a fabricated or
  sensor-error reading, never let through as a real test-result fact."
  [um]
  (and (number? um)
       (>= (double um) tolerance-test-um-min)
       (<= (double um) tolerance-test-um-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch machining/heat-treatment/
  grinding defect-rate reading? Rejects nil, non-numbers, negative
  values, and values beyond `defect-rate-max-percent` -- a fabricated
  or sensor-error reading, never let through as a real batch fact."
  [percent]
  (and (number? percent)
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a bearing/gear tolerance-class certification
  mark -- this actor is never the certification authority (see
...
(defn register-maintenance
  ...
  machining/grinding-line-equipment maintenance window against a
  verified, registered piece of equipment. Pure function -- does not
  actuate the machining/grinding-line equipment or execute any
  maintenance; it builds the RECORD a plant coordinator would keep.
  `beargearmfg.governor` independently re-verifies the equipment's own

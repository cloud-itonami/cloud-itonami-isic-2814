(ns beargearmfg.operation
  "BearGearOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (BearGearAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Bearings, Gears and
  Driving Elements Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [beargearmfg.advisor :as advisor]
...
(defn build
  "Compiles a BearGearOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `beargearmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BearGearAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Bearings, Gears and Driving Elements Plant Operations Governor -- independent censor.
      (g/add-node :govern

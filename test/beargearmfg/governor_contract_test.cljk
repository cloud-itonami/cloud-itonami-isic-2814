(ns beargearmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control machining/grinding-line equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations... does NOT self-issue a bearing/gear tolerance-class
  certification mark') implemented faithfully. The single invariant
  under test:

    BearGearAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Bearings, Gears and Driving
    Elements Plant Operations Governor would reject;
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :ball-bearing}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :ball-bearing (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-machining-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the bearing/gear tolerance-class certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-tolerance-test-um-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:tolerance-test-um 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-tolerance-test-um} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:tolerance-test-um (store/batch db "batch-001"))) "fabricated tolerance-test-um never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "mach-001" :severity :moderate
                                            :description "spindle vibration anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "mach-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
